...
const LiveChat = ({ currentShow, getRewardEarningAmount }) => {
	const { message, sendMessage, page, setPage, totalCount, loading } =
		useLiveChat(currentShow);
	...
	const getProfile = async () => {
		if (user && user.userId) {
			Api.viewUserProfile(user.userId, "edit-profile")
				.then((result) => {
	...
	const getFormData = ({ typedMessage }) => {
		...
		const sentMessage = {
			...
			reply:
				messageForReply && JSON.stringify(messageForReply) !== "{}"
					? messageForReply
					: null,
	...
	return (
		<div className='rounded-2xl py-5 sm:py-7 px-6 sm:px-8 bg-[#010101]'>
			<ScrollToBottom>
				<div
					className='space-y-4 mb-6 h-[300px] pr-2 pb-2'
					ref={scrTop}>
					{loading ? (
								...
								return (
									<Fragment key={index}>
										...
										<div
											...
											<p
												className='text-xs md:text-sm font-medium px-2 py-1 rounded-xl'
												style={{
													background: "#f9ff00",
													color: "#000",
												}}>
