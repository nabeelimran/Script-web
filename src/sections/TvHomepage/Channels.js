...
function Channels({
  ...
  useEffect(() => {
    ...
    let cursorint = setInterval(() => {
      ...
      if (
        todayDate.getMinutes() === 30 ||
        todayDate.getMinutes() == "00" ||
        todayDate.getMinutes() === 0
      ) {
        let timelinedata = helper.createTimeSlot(new Date());
        setTimeline(timelinedata);
  ...
  const getDurationInMinute = (startedAt, endedAt) => {
    ...
    return {
      ...
      time: `${helper.getIn12HoursFormat(
        startDate
  ...
  const saveVideoDuration = async (e) => {
    ...
    const req = {
      ...
      glassId:
        selectedGlass && selectedGlass?.sessionId ? selectedGlass?.glassId : 0,
    };
  ...
  const getChannelByChannelId = async (chn = undefined) => {
    return await Api.getChannelDetailByChannelId(
      chn ? chn.id : selectedChananel.id,
      false,
      userId,
      "watch"
    ).then((res) => {
        ...
        <div className="mb-6">
          <div className="grid lg:grid-cols-[1fr_340px] gap-10 items-center mb-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <>
                {userId && selectedGlass && selectedGlass?.sessionId ? (
                  <EndRecaptchaPopup
                    open={modal}
                    setOpen={setModal}
                ...
                <Button
                  type="button"
                  label={
                    selectedGlass && selectedGlass?.sessionId
                      ? "End Session"
                      : recaptchaCode
                  }
          ...
          <div className="grid xl:grid-cols-[1fr_340px] gap-10 items-center">
            <div className="xl:flex items-center space-y-12 xl:space-y-0 xl:space-x-6">
              <div className="grid grid-cols-2 xl:grid-cols-[110px_110px] gap-4 xl:gap-6">
                <GlassModalButton
                  selectedChananel={selectedChananel}
                  user={user}
                  selectedGlass={selectedGlass}
                ...
                <SquareBox className="flex-1 xl:flex-auto">
                  ...
                  />
                  <div
                    className="text-xs xl:text-sm bg-black font-medium lh-1_2 rounded text-center"
                    onClick={() => {
